...
export default {
  ...
  computed: mapState({
    nodes: state => state.graphData.aggregatedNodes,
    links: state => state.graphData.aggregratedLinks
  }),
  mounted () {
    this.initChart()
  },
  methods: {
    initChart () {
      this.$store.dispatch('GetAggregatedLinks').then(data => {
        this.$store.dispatch('GetAggregatedCompanies').then(data => {
